  ...
  return (
    <div style={{ background: "#071a0e", minHeight: "100vh", fontFamily: "sans-serif", color: "white" }}>
      ...
      <div style={{ padding: "40px 48px", borderTop: "0.5px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "inline-block", background: "rgba(74,222,128,0.12)", color: "#4ade80", fontSize: "10px", letterSpacing: "1px", textTransform: "uppercase", padding: "4px 10px", borderRadius: "4px", marginBottom: "12px", border: "0.5px solid rgba(74,222,128,0.3)" }}>Global reach</div>
        <h2 style={{ fontSize: "26px", fontWeight: 500, letterSpacing: "-0.5px", marginBottom: "6px" }}>From Ecuador to the world.</h2>
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px", lineHeight: 1.6, marginBottom: "28px" }}>Consolidated LCL containers from origin to major ports in Europe, North America, and Asia.</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "28px" }}>
          <div>
            <div style={{ color: "rgba(255,255,255,0.35)", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "12px" }}>Origin</div>
            {[
              { port: "Guayaquil, Ecuador", desc: "Main export hub · Shrimp, fruit, cacao" },
              { port: "Manta, Ecuador", desc: "Pacific coast · Tuna, fresh fish" },
            ].map(o => (
              <div key={o.port} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "14px", background: "#0a2414", border: "0.5px solid rgba(74,222,128,0.18)", borderRadius: "9px", marginBottom: "8px" }}>
                <div style={{ width: "7px", height: "7px", background: "#4ade80", borderRadius: "50%", flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: "13px", fontWeight: 500 }}>{o.port}</div>
                  <div style={{ color: "rgba(255,255,255,0.38)", fontSize: "11px", marginTop: "2px" }}>{o.desc}</div>
                </div>
              </div>
            ))}
          </div>
          <div>
            <div style={{ color: "rgba(255,255,255,0.35)", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "12px" }}>Destination ports</div>
            {[
              { port: "Rotterdam", country: "Netherlands", region: "EU", color: "#60a5fa" },
              { port: "Valencia", country: "Spain", region: "EU", color: "#60a5fa" },
              { port: "Hamburg", country: "Germany", region: "EU", color: "#60a5fa" },
              { port: "Miami", country: "United States", region: "USA", color: "#f472b6" },
              { port: "Los Angeles", country: "United States", region: "USA", color: "#f472b6" },
              { port: "Tokyo", country: "Japan", region: "Asia", color: "#fb923c" },
              { port: "Shanghai", country: "China", region: "Asia", color: "#fb923c" },
            ].map(d => (
              <div key={d.port} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "#0a2414", border: "0.5px solid rgba(255,255,255,0.05)", borderRadius: "9px", marginBottom: "5px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: d.color, flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: "12px" }}>{d.port}</div>
                    <div style={{ color: "rgba(255,255,255,0.35)", fontSize: "10px" }}>{d.country}</div>
                  </div>
                </div>
                <span style={{ fontSize: "10px", padding: "2px 7px", borderRadius: "4px", background: `${d.color}1a`, color: d.color, border: `0.5px solid ${d.color}33` }}>{d.region}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* FOOTER */}
      <div style={{ borderTop: "0.5px solid rgba(255,255,255,0.06)", padding: "28px 48px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
